function [] = YAGI_V2
% CODIGO PARA ANTENAS Y RADIO PROGACION STEVEN KNAUTH V2 %
clear all;
clc;
...
while I <= (M * N)
    ...
    while J <= (M * N)           
        ...
        RES = SINTEG (UL, LL, 10);  %     Realiza la integración numérica
        LEN = L (N2) / 2;
        G2 = KERNEL (LEN);
        F2M = NMODE * 2 - 1;
...
while J <= NO
    ...
    while I <= 16
        X = S + GAUSS(I) * DEL;
        SUM = SUM + LEGEND(I) * FF(X); % Evalúa la función FF en el punto X y suma ponderada
        I = I + 1;
    end
    J = J + 1;
end
